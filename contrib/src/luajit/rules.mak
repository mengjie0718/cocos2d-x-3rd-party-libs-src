...
LUAJIT_TARGET_FLAGS="${EXTRA_CFLAGS} ${EXTRA_LDFLAGS} -fPIC"
...
ifeq ($(MY_TARGET_ARCH),x86_64)
	...
	cd $< && CFLAGS="-DLUAJIT_DISABLE_GC64" LD_FLAGS="" $(MAKE) MACOSX_DEPLOYMENT_TARGET=$(OSX_VERSION) -j8
endif
...
ifeq ($(MY_TARGET_ARCH),x86_64)
	cd $< && CFLAGS="-DLUAJIT_ENABLE_GC64" LD_FLAGS="" $(MAKE) MACOSX_DEPLOYMENT_TARGET=$(OSX_VERSION) -j8 HOST_CC=$(LUAJIT_HOST_CC) CROSS=$(LUAJIT_CROSS_HOST) TARGET_SYS=iOS  TARGET_FLAGS=$(LUAJIT_TARGET_FLAGS)
else
	cd $< && $(MAKE) MACOSX_DEPLOYMENT_TARGET=$(OSX_VERSION) -j8 HOST_CC=$(LUAJIT_HOST_CC) CROSS=$(LUAJIT_CROSS_HOST) TARGET_SYS=iOS  TARGET_FLAGS=$(LUAJIT_TARGET_FLAGS)
endif
